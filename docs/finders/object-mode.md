...
Object-mode finders run in **Object Mode** and select whole **objects** that match a
trait. They are grouped into the categories below as they appear in the FindMe panel.
A ⚙ marks finders with tunable [settings](../settings.md).
...
: *Max connected edges* — flag the mesh if any vertex exceeds this count.
...
: *Allow empty mirror objects* — treat `EMPTY` mirror objects as valid.

By Modifier ⚙
: Selects objects that have a modifier of a chosen type. The button label reflects the chosen type.
: *Modifier type* — modifier type to search for (clearing it disables the finder).

By GN Tree ⚙
: Selects objects with a Geometry Nodes modifier using a chosen node tree. The button label reflects the chosen tree.
